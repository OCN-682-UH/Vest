...
This document contains two custom R functions for data preparation and initial visualization for my final project (and future research) data
...
A plotting function (plot_bar_means) that creates a basic bar chart comparing group means
...
# Data preparation: Extract the sample type (e.g., 'C' or 'V') from CowTagID
# This is our grouping factor for the study
data <- data %>%
  mutate(
    Sample_Type = str_extract(CowTagID, "^[A-Z]+"),
    # Create a simple ratio column for plotting flexibility
...
#A new data frame with N, Mean, and SD.
summarize_data_by_group <- function(data, group_col, var_col) {
  
  # Ensure the grouping column is treated as a factor for correct summarization
...
## Function 2
The plot_bar_means function takes the output of the summary function above and quickly visualizes the mean of the variable across groups using a bar chart, including standard deviation as error bars to represent variability.

```{r}
#| label: plot-function
#| echo: true

#' Creates a Bar Plot of Group Means with SD Error Bars
#'
#' @param summary_data A data frame outputted by the summarize_data function.
#' @param y_label The label for the y-axis (as a string).
